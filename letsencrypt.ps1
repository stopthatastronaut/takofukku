...
Set-AWSCredentials -AccessKey $env:AWS_ACCESS_KEY_ID -SecretKey $env:AWS_SECRET_ACCESS_KEY -verbose
...
Write-Host "Sleeping for a few seconds to be sure R53 is updated"
...
Get-ACMECertificate $certname -ExportPkcs12 $pfxfile -CertificatePassword $env:takofukku_cert_password
...
Set-AzContext -SubscriptionId $env:az_subscription_id_stopthatastronaut

New-AzWebAppSSLBinding `
    -WebAppName Takofukku `
    -ResourceGroupName Takofukku `
    -Name hook.takofukku.io `
    -CertificateFilePath $pfxfile `
    -CertificatePassword $env:takofukku_cert_password `
    -SslState SniEnabled `
    -Verbose
